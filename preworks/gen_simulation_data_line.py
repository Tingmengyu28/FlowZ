...
with open(txt_file_path, 'w') as txt_file:
    for stack_idx in trange(1, num_stacks + 1):
        ...
        curve_type = random.choice(['line', 'quadratic', 'cubic'])
        
        if curve_type == 'line':
            start_point = (random.randint(30, img_size-30), 
                          random.randint(30, img_size-30), 
                          0)
            end_point = (random.randint(30, img_size-30), 
                        random.randint(30, img_size-30), 
                        num_slices-1)
            ...
            for i in range(num_points):
                ...
                px = int(start_point[0] + t * (end_point[0] - start_point[0]))
                py = int(start_point[1] + t * (end_point[1] - start_point[1]))
                pz = int(start_point[2] + t * (end_point[2] - start_point[2]))
                px = max(30, min(img_size-30, px + random.randint(-5, 5)))
                py = max(30, min(img_size-30, py + random.randint(-5, 5)))
                pz = max(0, min(num_slices-1, pz + random.randint(-1, 1)))
            ...
            end_point = (random.randint(30, img_size-30), 
                        random.randint(30, img_size-30), 
                        num_slices-1)
            control_point = (random.randint(30, img_size-30), 
                            random.randint(30, img_size-30), 
                            random.randint(0, num_slices-1))
            
            points = []
            for i in range(num_points):
                t = i / (num_points - 1) if num_points > 1 else 0.5
                px = int((1-t)**2 * start_point[0] + 2*t*(1-t) * control_point[0] + t**2 * end_point[0])
                py = int((1-t)**2 * start_point[1] + 2*t*(1-t) * control_point[1] + t**2 * end_point[1])
                pz = int((1-t)**2 * start_point[2] + 2*t*(1-t) * control_point[2] + t**2 * end_point[2])
                px = max(30, min(img_size-30, px + random.randint(-5, 5)))
                py = max(30, min(img_size-30, py + random.randint(-5, 5)))
                pz = max(0, min(num_slices-1, pz + random.randint(-1, 1)))
                points.append((px, py, pz))
            ...
            end_point = (random.randint(30, img_size-30), 
                        random.randint(30, img_size-30), 
                        num_slices-1)
            control_point1 = (random.randint(30, img_size-30), 
                             random.randint(30, img_size-30), 
                             random.randint(0, num_slices-1))
            control_point2 = (random.randint(30, img_size-30), 
                             random.randint(30, img_size-30), 
                             random.randint(0, num_slices-1))
            ...
            for i in range(num_points):
                t = i / (num_points - 1) if num_points > 1 else 0.5
                px = int((1-t)**3 * start_point[0] + 3*t*(1-t)**2 * control_point1[0] + 
                        3*t**2*(1-t) * control_point2[0] + t**3 * end_point[0])
                py = int((1-t)**3 * start_point[1] + 3*t*(1-t)**2 * control_point1[1] + 
                        3*t**2*(1-t) * control_point2[1] + t**3 * end_point[1])
                pz = int((1-t)**3 * start_point[2] + 3*t*(1-t)**2 * control_point1[2] + 
                        3*t**2*(1-t) * control_point2[2] + t**3 * end_point[2])
                px = max(30, min(img_size-30, px + random.randint(-5, 5)))
                py = max(30, min(img_size-30, py + random.randint(-5, 5)))
                pz = max(0, min(num_slices-1, pz + random.randint(-1, 1)))
                points.append((px, py, pz))
